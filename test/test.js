...
const Game = new Mafia.Engine({
     ...
     rolelist: ['Lookout', 'Town Protective', 'Sheriff', 'Random Town', 'Plague', 'Evil Killing'],
...
Game.phases.set("Day", {duration: 10, first: true, next: 'Night', lynchPlayer: false});
Game.phases.set("Night", {duration: 29, next: 'Day', evaluateActions: true});
...
Game.roles.set("Doctor", {side: "Town", alignment: "Protective", action: (doer, target) => {
    ...
    doer.roleAttrs.heals--;
}}).addAttr('heals', 3)
Game.roles.set("Wolf", {side: "Evil", alignment: "Killing", factionalAction: true, action: (doer, target) => {
      if (!target.protected) target.kill(doer);
      else console.log(`${doer}: You couldn't attack ${target}! They were healed! The doctor now has ${Game.players.find(p => p.role.name == 'Doctor').roleAttrs.heals} heals!`)
}});
Game.roles.set("Plague", {side: "Evil", alignment: "Support", visits: false, action: (doer, target) => {
   if (doer.roleAttrs.things == 0)  return console.log(`${doer}: You don't have anymore things!`);
   target.protected = false;
   doer.roleAttrs.things--;
}}).addAttr("things", 2);
...
Game.on("Day", phase => {
    ...
    for (let [, player] of Game.players.alive()) {
