...
class RoleCollection extends Collection {
    ...
    set(name, data = {alignment: null, side: null, unique: false, amount: 999, blocked: false, action: null, visits: true}) {
   ...
   setRules(fn) {
    ...
    fromSide(side) {
          ...
          do {
            ...
            if (!tried.includes(trythis.name) && trythis.amount >= 1 && !trythis.blocked && this.rules(trythis, side)) {
                 role = trythis;
    ...
    fromAlign(side, alignment) {
        ...
        do {
            if (this.size == 0) break;
            const trythis = possible.random();
            if (!tried.includes(trythis.name) && trythis.amount >= 1 && !trythis.blocked && this.rules(trythis, side, alignment)) {
                 role = trythis;
                if (trythis.unqiue) this.delete(trythis.name);
                trythis.amount--;
                 if (trythis.amount <= 0) this.delete(trythis.name);
