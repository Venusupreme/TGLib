...
class RoleCollection extends Collection {
    ...
    set(name, data = {alignment: null, side: null, unique: false, amount: 999, blocked: false, action: null, visits: true, factionalAction: false}) {
    ...
    parseSet(obj) {
       ...
       for (let key in obj) {
           const val = obj[key];
           if (defaultAttrs.includes(key)) role[key] = val;
           else role.addAttr(key, val);
       }
        const backup = role.clone();
        super.set(role.name, role);
        this.original.set(backup.name, backup);
        this.engine.sides.has(role.side) ? this.engine.sides.get(role.side).roles.set(name, backup):this.engine.sides.set(role.side, {roles: [backup]});
        role.side = this.engine.sides.get(role.side);
        backup.side = this.engine.sides.get(role.side);
        return role;
    }
