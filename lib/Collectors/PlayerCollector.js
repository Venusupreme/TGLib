...
class PlayerCollector extends Unit {
    ...
    clearActions() {
        ...
        for (let [, side] of this.engine.sides) side.factionAction.clear()
    }

    executeActions() {
        for (let role of this.engine.roles._orderRolesByPriorityCache) {
             const players = this.filter(p => p.action && p.role.name === role.name && !p.roleblocked && p.role.action && !p.invisible);
