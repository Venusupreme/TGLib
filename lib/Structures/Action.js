...
class Action {
    ...
    clear() {
        if (this.factionAction) this.player.role.side.factionAction = null;
        this.player.action = null;
    }

    /**executes the action */
    exe() {
        if (this.target && this.player.role.interactions.has(this.target.role.name)) return this.player.role.interactions.exe(this.target.role.name, this.player, this.target, this.other);
