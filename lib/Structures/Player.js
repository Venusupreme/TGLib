...
class Player {
    ...
    kill(killer) {
        this.dead = true;
        if (killer) this.engine.events.emit("kill", this, killer);
    }
    ...
    setAction(target, data = {factionAction: false}) {
        this.action = new Action({target: target, player: this, data: data});
        this.action._checkForFA();
    ...
    get visitors() {
        ...
        return this.engine.players.filter(p => p.action && p.action.target && p.action.target.name === this.name && p.role.visits);
    }
    ...
    setVotePower(value) {
        ...
        this.votePower = value;
        this.engine.events.emit("setVotePower", this);
    }
